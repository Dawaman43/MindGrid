...
export const nodeRelations = relations(node, ({ one, many }) => ({
  ...
  outgoingEdges: many(edge, { relationName: "edge_source_node" }),
  incomingEdges: many(edge, { relationName: "edge_target_node" }),
  tags: many(nodeTag),
}));
...
export const edgeRelations = relations(edge, ({ one }) => ({
  ...
  sourceNode: one(node, {
    ...
    references: [node.id],
    relationName: "edge_source_node",
  }),
  targetNode: one(node, {
    fields: [edge.targetNodeId],
    references: [node.id],
    relationName: "edge_target_node",
  }),
}));
